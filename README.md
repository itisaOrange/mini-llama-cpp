...
一个可读型的 LLaMA-style C++ 推理内核。从 MVP 的 toy 模型出发，逐步构建成一个可读、可测、可解释的推理引擎，并提供终端聊天体验。
...
- `Qwen2.5-0.5B-Instruct-Q4_0.gguf`：可加载并完成 `generate -n 1`。
- `SmolLM2-135M-Instruct-Q4_0.gguf`：可加载并完成 `generate -n 1`。
...
mini-llama.cpp/
  ...
  include/mini_llama/
    ...
    request_context.h # 请求级 trace_id、阶段耗时、token 统计、错误信息
  ...
  src/
    ...
    cuda_kv_cache.cpp / cuda_kv_cache.cu
...
- RequestContext: trace_id、trace event、trace summary、错误状态格式
